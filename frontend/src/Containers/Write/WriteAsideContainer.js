...
const WriteAsideContainer = ({ setModalOn, selectedFiles, setSelectedFiles }) => {
    ...
    useEffect(() => {
        ...
        if (isEdit) {
            ...
            setDropdownState(category === 'info' ? '정보 공유' : category === 'study' ? '스터디 해요' : 'Q&A');
        ...
        <section className="aside col-span-5 col-start-11 ml-[15px]">
            <section className="border_primary flex flex-col rounded-[12px] border">
                ...
                <div className="px-[20px]">
                    {true ? (
                        <ExistAttachFile selectedFiles={selectedFiles} onClickDelete={onClickDelete} />
                    ) : (
...
const AsideMenuTitle = ({ text, isFile, setModalOn, selectFile, inputFileRef }) => {
    return (
        <div className="surface_secondary border_primary flex h-[54px] items-center justify-between border-y px-[16px] first:rounded-t-[12px] first:border-t-0">
            ...
            {isFile && (
                <>
                    <input type="file" onChange={selectFile} style={{ display: 'none' }} ref={inputFileRef} multiple />
                    <button
                        aria-label="button"
                        type="submit"
