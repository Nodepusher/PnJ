...
const MyPageEditContainer = ({ user, profile }) => {
    ...
    useEffect(() => {
        setValid({
            passwordValid: updateUserInfo.password
                ? /^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{8,}$/.test(updateUserInfo.password)
                : null,
            passwordMatch:
    ...
    const handleSave = async (e) => {
        ...
        if (canUpdate) {
            ...
            try {
                // 서버에 업데이트 요청 보내기
                await axios.put('/user/updateUserInfo', formData, {
                    headers: {
  ...
  */
    return (
        <>
            <div className="shrink-0 grow basis-0 overflow-x-auto">
                <div className="mx-auto grid w-desktop-grid grid-cols-12 gap-x-[16px] gap-y-0 pb-[64px]">
                    <div className="col-span-full">
                        <form onSubmit={handleSave} method="PUT" encType="multipart/form-data">
                            <EditInfo />
                            <div>
                                <div className="mx-auto grid w-desktop-grid grid-cols-12 gap-x-[16px] gap-y-0 pb-[64px]">
            ...
            {isSaved && <SaveInfo animationClass={animationClass} updateState={updateState} />}
        </>
    );
};
