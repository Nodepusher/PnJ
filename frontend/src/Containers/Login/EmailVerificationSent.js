...
const EmailVerificationSent = ({ sendMailState, isValidCallback }) => {
    ...
    if (sendMailState) {
        content = '메일이 오지 않을 경우 스팸 메일로 분류되어 있을 수 있으니, 스팸 메일 보관함을 확인해 주세요.';
    } else {
        content =
            ...
            {sendMailState && (
                <div className="relative mx-auto flex w-full rounded-[8px] p-[16px] surface_positive_light">
                    ...
                    <div className="flex flex-col items-start">
                        <strong className="content_primary font_label_bold_lg mb-[4px]">인증메일 발송 완료</strong>
                        <p className="content_secondary font_body_regular_md">
                            <span className="font_label_bold_md">b7309@naver.com</span>으로 인증메일을 발송했습니다.
                            메일을 확인해주세요.
                        </p>
                    </div>
                </div>
            )}
            <p className="content_secondary font_body_regular_md keep-all mt-[24px] text-center">{content}</p>
            {!sendMailState && (
                <div className="mt-[40px]">
                    <div>
                        ...
                        />
                        <UserInputMsg isValid={validationResults.emailValid} text={'올바른 이메일 형식이 아닙니다.'} />
                    </div>
                </div>
            )}
        </>
