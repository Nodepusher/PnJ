...
const EmailSignUpContainer = () => {
  ...
  const signUp = async (sendData) => {
    const URL = "http://localhost:4000/user/create-account";
    return axios
      .post(URL, {
        data: sendData,
      })
  ...
  const onSubmit = async (e) => {
    ...
    const allValuesTrue = Object.values(validationResults).every(
      (value) => value === true
    );
  ...
  const handleCheckBoxChange = useCallback(
    (e, index) => {
      if (typeof index === "number") {
        ...
        setOptions(
          options.map((option) => ({ ...option, isChecked: e.target.checked }))
        );
      }
    },
    [options]
