...
const DetailSectionContainer = () => {
    ...
    const profile = post.User ? `/uploads/file/${post.User.profile}` : `${defaultImageUrl}`;
    const { title, tag, createdAt } = post;
    const category = post.category === 'info' ? '정보 공유' : post.category === 'study' ? '스터디해요' : 'Q&A';
    ...
    return (
        <div className="py-[16px] px-[16px]">
            <main className="min-w-screen surface_primary relative mx-auto flex w-full min-w-[320px] max-w-main flex-col">
                ...
                <DetailReplyContainer profile={profile} />
                <div className="h-[28px]"></div>
                <div className="border-t border_primary"></div>
                <div className="h-[28px]"></div>
