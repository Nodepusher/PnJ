...
const ReplyComponent = ({ showReply, hideReply, StBtn, StImg, replies, formatDateTime, profile, loginUser, post }) => {
    ...
    const removeReply = async (replyId) => {
        try {
            const response = await axios.delete(`/board/deleteReply/${replyId}`);
            ...
            <div>
                                ...
                                <div className="w-full">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-[4px]">
                                            <div className="content_primary font_label_bold_lg">{reply.User.name}</div>
                ...
                {showReply && (
                    <div className="reply-input-section">
                        <InputCommentComponent type={type} replyCallback={replyCallback} profile={profile} />
                    </div>
                )}
            </div>
        </div>
