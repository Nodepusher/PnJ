...
module.exports = {
    ...
    getAllForInfiniteScroll: async (req, res, next) => {
        ...
        try {
            const board = await boardService.findAllForInfiniteScroll(limit, page, category, sort);
            if (!board) {
    ...
    deleteMyComment: async (req, res) => {
        try {
            ...
            if (result.success) {
                return res.status(200).json({ success: true, message: '댓글 삭제 성공' });
            } else {
    ...
    deleteMyReply: async (req, res) => {
        try {
            ...
            if (result.success) {
                return res.status(200).json({ success: true, message: '대댓글 삭제 성공' });
            } else {
                res.status(400).json({ success: false, message: '대댓글 삭제 실패' });
            }
        } catch (error) {
            res.status(500).json({
                success: false,
